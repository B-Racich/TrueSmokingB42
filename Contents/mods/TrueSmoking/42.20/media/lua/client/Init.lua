...
function TrueSmoking.initPlayer(playerNum, player)
    ...
    -- Reconnect/character creation repair: nothing should occupy the dedicated
    -- smoking body location if the session was just reset.
...
--------------------------------------------------------------------------------
-- Stale Visual Watchdog
--------------------------------------------------------------------------------

-- Last-resort self-healing: if state says the player isn't smoking, the
-- dedicated body location must not still hold a True Smoking visual.
local staleVisualTicks = {}
local STALE_VISUAL_CHECK_INTERVAL = 30

Events.OnPlayerUpdate.Add(function(player)
    if not player or not TrueSmoking.registries or not TrueSmoking.registries.mask then return end

    local key = player:getPlayerNum()
    staleVisualTicks[key] = (staleVisualTicks[key] or 0) + 1
    if staleVisualTicks[key] < STALE_VISUAL_CHECK_INTERVAL then return end
    staleVisualTicks[key] = 0

    local data = TrueSmoking.Data.getSmoking(player)
    if data and not data.isSmoking then
        local worn = player:getWornItem(TrueSmoking.registries.mask)
        if worn then
            TrueSmoking.debug('Watchdog removed stale smoking visual')
            TrueSmoking.Visuals.removeMask(player)
        end
    end
end)
