...
function SmokableItem:stop()
    ...
    -- Every real stop must remove the dedicated smoking body-location visual.
    if player then
        TrueSmoking.Visuals.removeMask(player)
    end

    -- Drop the stale reference so later MP failure detection isn't ambiguous.
    local ref = player and TrueSmoking.getPlayerRef(player)
    if ref and ref.smokable == self then
        ref.smokable = nil
    end
end
...
function SmokableItem:update(player)
    ...
    if not currentItem then
        -- End the whole session so a transient inventory move can never leave
        -- a ghost visual (a bare state flip here is not enough to clean it up).
        TrueSmoking.debug('SmokableItem:update - active smokable missing from player containers; stopping cleanly')
        self:stop()
        return
    end
    ...
    -- KeepLit: hold the ember above the burnout threshold while a non-smoking
    -- action blocks auto-puffing, so a busy player can't lose the cigarette.
    if TrueSmoking.Config and TrueSmoking.Config.KeepLit and not self:canAutoPuff() then
        if self.burnRate < 0.00001 then
            self.burnRate = 0.00001
        end
    end
