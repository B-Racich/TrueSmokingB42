...
--------------------------------------------------------------------------------
-- Moodle Framework 2.7 Constructor Hotfix (B42.20)
--------------------------------------------------------------------------------

-- MF 2.7's B42.20 branch creates the new ISMoodle object (`o`), assigns
-- o.name/o.char, but initializes storage via MF.ISMoodle:getMoodleData()
-- instead of o:getMoodleData(). That reads nil name/char on the class table
-- itself, which can fail OnCreatePlayer before MF.MoodlesStorage is
-- populated, leaving MF.getMoodle() permanently nil for our moodles.
-- Temporarily exposing the constructor args on the class lets the upstream
-- 2.7 constructor complete unchanged.
if MF and MF.ISMoodle and MF.ISMoodle.new and not MF.ISMoodle._TrueSmokingCtorHotfix then
    local originalMoodleNew = MF.ISMoodle.new
    MF.ISMoodle._TrueSmokingCtorHotfix = true
    ...
    function MF.ISMoodle:new(moodleName, character)
        ...
        local ok, result = pcall(originalMoodleNew, self, moodleName, character)
...
function TrueSmoking.updateSmokingMoodle(player)
    ...
    -- Hide the inactive style moodle. With MF 2.7, nil means OnCreatePlayer
    -- has not created it yet; simply try again on the next update tick.
    local inactiveMoodle = MF.getMoodle(inactiveId, playerNum)
    if inactiveMoodle and inactiveMoodle.setValue then
        inactiveMoodle:setValue(0.5)
    end

    local moodle = MF.getMoodle(activeId, playerNum)
    if not moodle or not moodle.setThresholds or not moodle.setValue then return end
    ...
    local originalSmokeLength = tonumber(smokable.originalSmokeLength) or 0
    if originalSmokeLength <= 0 then
        moodle:setValue(0.5)
        return
    end

    local percent = (tonumber(smokable.smokeLength) or 0) / originalSmokeLength
    ...
    if moodle.setDescription and moodle.getGoodBadNeutral and moodle.getLevel then
...
function TrueSmoking.updateNicotineMoodle(player)
    ...
    if shouldShow and not hideMoodles then
        ...
        local level = moodle.getLevel and moodle:getLevel() or 0
        local gbn = moodle.getGoodBadNeutral and moodle:getGoodBadNeutral() or 0

        if moodle.setTitle and level > 0 and gbn ~= 0 then
