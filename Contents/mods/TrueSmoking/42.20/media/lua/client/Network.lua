...
function TrueSmoking.onServerCommand(module, command, args)
    ...
    if command == 'forceCleanup' then
        -- Server requested cleanup after a stale/nil ISUnequipAction packet.
        TrueSmoking.debug('Server requested force cleanup after stale unequip action')
