...
-- Cleanup must always be allowed even when ManageHeadGear is disabled: that
-- option only prevents creation of new visuals, never their removal.
-- @param player IsoPlayer
function TrueSmoking.Visuals.removeMask(player)
    if not player then return end

    -- MP: the server owns worn-item state; request cleanup unconditionally.
    if isClient() then
        sendClientCommand(player, 'TrueSmoking', 'removeVisualItem', {
            options = { ManageHeadGear = true }
        })
        return
    end
