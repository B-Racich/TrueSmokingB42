...
function LightSmoke:perform()
    ...
    -- Abort paths below never reach ISBaseTimedAction.perform() further down,
    -- which would otherwise leave this action stuck in the timed-action queue
    -- forever (an MP InventoryItem reference that never resolves, etc).
    local function abortCleanly(reason)
        TrueSmoking.debug(reason)

        local data = TrueSmoking.Data.getSmoking(self.character)
        if data then
            data.isSmoking = false
            data.takingPuff = false
            data.pendingCigaretteId = nil
        end

        ISBaseTimedAction.perform(self)
        TrueSmoking.Visuals.removeMask(self.character)

        if isClient() then
            sendClientCommand(self.character, 'TrueSmoking', 'updatePlayerData', {
                { isSmoking = false, takingPuff = false, pendingCigaretteId = nil }
            })
        end
    end
    ...
    if self.cigPack then
        ...
        if isClient() then
            ...
            if not itemToSmoke then
                return abortCleanly('LightSmoke:perform - MP: Could not find created cigarette, aborting')
            end
        else
            -- SP: Use cigarette created in start()
            itemToSmoke = self.cigarette
            if not itemToSmoke then
                return abortCleanly('LightSmoke:perform - SP: Cigarette not created, aborting')
            end
        end
    else
        ...
        if not itemToSmoke then
            return abortCleanly('LightSmoke:perform - No valid item reference found, aborting')
        end
    end
    ...
    if not ref.smokable then
        return abortCleanly('LightSmoke:perform - Failed to create smokable, aborting')
    end
...
function LightSmoke:complete()
    -- Don't create a visual if the client-side smoking object failed to
    -- initialize (MP start race); the queue was already released in perform().
    if isClient() then
        local ref = TrueSmoking.getPlayerRef(self.character)
        local data = TrueSmoking.Data.getSmoking(self.character)
        if not (ref and ref.smokable and data and data.isSmoking) then
            TrueSmoking.debug('LightSmoke:complete - suppressed stale visual/state after failed MP start')
            if data then
                data.isSmoking = false
                data.takingPuff = false
                data.pendingCigaretteId = nil
