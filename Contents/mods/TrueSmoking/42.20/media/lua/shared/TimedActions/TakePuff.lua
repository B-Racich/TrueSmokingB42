...
function TakePuff:start()
    ...
    if not self.item then
        TrueSmoking.endSmokingCleanly(self.character, 'TakePuff lost active item reference; ending smoking session')
        self:forceStop()
        return
    end
...
function TakePuff:perform()
    ...
    -- Only restore the visual if this puff actually removed it.
    if self.visualItemFlag and self.hasVisual and not isClient() and not isServer() then
        local current = self.character:getWornItem(TrueSmoking.registries.mask)
        if current then
            self.character:removeWornItem(current)
        end
