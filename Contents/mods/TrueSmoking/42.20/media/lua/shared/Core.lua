...
--------------------------------------------------------------------------------
-- Smoking Session Cleanup
--------------------------------------------------------------------------------

--- End an active smoking session cleanly (stale item, blocked by headgear,
-- unrecoverable MP desync, etc). Prefers routing through the active
-- SmokableItem so its retry/visual state stays in sync; falls back to a
-- manual state reset when no SmokableItem is available.
-- @param character IsoPlayer
-- @param reason string|nil Debug reason logged when cleanup runs
function TrueSmoking.endSmokingCleanly(character, reason)
    if not character then return end
    if reason then TrueSmoking.debug(reason) end

    local ref = TrueSmoking.getPlayerRef and TrueSmoking.getPlayerRef(character) or nil
    if ref and ref.smokable and ref.smokable.stop then
        ref.smokable:stop()
        return
    end

    local data = TrueSmoking.Data and TrueSmoking.Data.getSmoking and TrueSmoking.Data.getSmoking(character) or nil
    if data then
        data.isSmoking = false
        data.takingPuff = false
    end

    if TrueSmoking.Visuals and TrueSmoking.Visuals.removeMask then
        TrueSmoking.Visuals.removeMask(character)
    end

    if isClient() then
        sendClientCommand(character, 'TrueSmoking', 'updatePlayerData', {
